...
public class DisapearingNumber5 {
  public static void main(String[] args) {
    ...
    int a[] = {1, 6, 5, 2, 1, 4};//Disappeared Number from the list :[3]
  ...
  public static List<Integer> findDisappearingNumber(int a[]) {
    ...
    for (int i = 0; i < size; i++) {
      //taking the element of an array as an indices and checking that indices is occupied or not
      //if occupied then marked it negative
      int ind = Math.abs(a[i]) - 1;
      System.out.println("Inf :" + ind);
      if (a[ind] < 0)
        continue;
      a[ind] = a[ind] * -1;
    }
